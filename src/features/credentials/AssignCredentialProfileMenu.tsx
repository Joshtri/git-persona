...
export function AssignCredentialProfileMenu({ credential }: Props) {
  const [open, setOpen] = useState(false);
  const profiles = useProfilesStore((s) => s.items);
  const assignProfile = useCredentialsStore((s) => s.assignProfile);

  const assigned = profiles.find((p) => p.id === credential.profile_id) ?? null;

  const choose = (profileId: string | null) => {
    setOpen(false);
    if (profileId !== credential.profile_id) {
      assignProfile(credential.id, profileId);
    }
  };
      ...
      <Popover.Portal>
        <Popover.Positioner sideOffset={6} align="end">
          <Popover.Popup className="z-50 min-w-44 rounded-(--radius-md) border border-(--color-border) p-1 shadow-(--shadow-md) outline-none bg-(--color-surface)">
            ...
            >
              Unassigned
              {credential.profile_id == null && <Check className="size-3.5" aria-hidden="true" />}
            </button>
